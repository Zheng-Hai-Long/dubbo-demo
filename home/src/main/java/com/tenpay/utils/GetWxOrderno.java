...
import com.tenpay.utils.http.HttpClientConnectionManager;
...
//    	  }
	     ...
	     client.getConnectionManager().shutdown();
	     System.out.println("************************************");
	     System.out.println("返回xml"+jsonStr);
	    if(jsonStr.indexOf("FAIL")!=-1){
	    	return code_url;
	    }
	    System.out.println("************************************");
	 ...
	 * @see
	 */
	public static String getMWebUrl(String url,String xmlParam){
		DefaultHttpClient client = new DefaultHttpClient();
   ...
   * @see
   */
  public static String getPayResult(String url,String xmlParam){
	  DefaultHttpClient client = new DefaultHttpClient();
	  client.getParams().setParameter(ClientPNames.ALLOW_CIRCULAR_REDIRECTS, true);
	  HttpPost httpost= HttpClientConnectionManager.getPostMethod(url);
	  String code_url = "";
...
//    	  }
	    ...
	    Map map = doXMLParse(jsonStr);
	    code_url  = (String) map.get("return_code")
	    			+"|"+(String) map.get("out_trade_no")
	    			+"|"+(String) map.get("transaction_id")
	    			+"|"+(String) map.get("attach");
	} catch (Exception e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
	}
